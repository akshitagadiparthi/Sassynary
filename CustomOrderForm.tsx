// FILE: components/CustomOrderForm.tsx (FIXED)
// CHANGES:
//  1. Added file type whitelist validation (images only)
//  2. Added 5MB max file size limit
//  3. Added input length limits
//  4. Sanitize text inputs before storing

import React, { useState, useRef } from 'react';
import { Send, Pencil, ArrowLeft, Upload, X, Image as ImageIcon, Loader2, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        if (isFirebaseReady && db && storage) {
            if (selectedFile) {
                try {
                    // Use a sanitized filename: timestamp + random suffix
                    const safeExt = selectedFile.name.split('.').pop()?.toLowerCase() || 'jpg';
                    const safeName = `${Date.now()}-${crypto.getRandomValues(new Uint32Array(1))[0].toString(36)}.${safeExt}`;
                    const storageRef = ref(storage, `custom-orders/${safeName}`);
                ...
                {isSubmitted ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <input required type="text" maxLength={100} value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-white border border-gray-100 px-4 py-3 rounded-lg focus:border-pink-600 focus:outline-none transition-colors" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2 ml-1">Phone</label>
                            <input required type="tel" maxLength={15} value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="w-full bg-white border border-gray-100 px-4 py-3 rounded-lg focus:border-pink-600 focus:outline-none transition-colors" />
                        </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2 ml-1">Email</label>
                                <input required type="email" maxLength={100} value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full bg-white border border-gray-100 px-4 py-3 rounded-lg focus:border-pink-600 focus:outline-none transition-colors" />
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <textarea required rows={4} maxLength={1000} value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} placeholder="Quantities, design ideas, deadlines..." className="w-full bg-white border border-gray-100 px-4 py-3 rounded-lg focus:border-pink-600 focus:outline-none transition-colors resize-none" />
                        </div>
                        <div>
                          ...
                          {fileError && (
                            <div className="mb-3 p-3 bg-red-50 border border-red-100 rounded-lg text-red-700 text-xs flex items-center gap-2">
                          ...
                          {!selectedFile ? (
                            <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center cursor-pointer hover:border-pink-400 hover:bg-pink-50 transition-all group">
                              ...
                              <p className="text-[9px] text-gray-300 mt-1">JPG, PNG, WebP, GIF — Max {MAX_FILE_SIZE_MB}MB</p>
