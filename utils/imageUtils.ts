...
export const getBlurPlaceholder = (url: string): string => {
  ...
  if (url.includes('imgur.com')) {
    try {
      ...
      return cleanUrl.replace(/(https?:\/\/i\.imgur\.com\/[a-zA-Z0-9]+)(\.[a-zA-Z]+)/, `$1s.webp`);
    } catch (e) {
      return url;
    }
  }
...
export const getOptimizedImageUrl = (url: string, width: number = 800) => {
  ...
  // 3. Fallback to Weserv Proxy for other hosts (with cache busting)
  try {
    return `https://images.weserv.nl/?url=${encodeURIComponent(url)}&w=${width}&q=80&output=webp&n=-1`;
  } catch (e) {
    return url;
  }
};
